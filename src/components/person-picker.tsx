...
/**
 * Everything about the control's size is fixed. The suggestions float above the
 * fields below them instead of pushing them down, and the chosen person sits in
 * a row exactly as tall as the search box, so nothing under the picker moves
 * when it opens or when a name is picked.
 */
...
export function PersonPicker({
  ...
  return (
    <div className="mt-6" ref={containerRef} data-testid="person-picker">
      {/* Both spellings are block-level and the same size, so swapping one for
          the other when a person is chosen cannot move anything. */}
      {selected ? (
        <p className="block text-xs font-semibold text-ink-muted">{label}</p>
      ) : (
        <label
          htmlFor={inputId}
      ...
      <div className="relative mt-1.5">
        {selected ? (
          <div
            className={clsx(
              "flex w-full items-center gap-3 rounded-2xl bg-porcelain pl-2.5 pr-1.5",
              controlHeight,
            )}
          >
            <Avatar
          ...
          <>
            ...
            <input
              ...
              className={clsx(
                "w-full rounded-2xl border border-black/10 bg-white pl-10 pr-4 text-sm text-ink outline-none transition focus:border-coral focus:ring-2 focus:ring-coral/20",
                controlHeight,
              )}
            />
          </>
        ...
        {open && !selected ? (
          <ul
            id={listId}
            role="listbox"
            className="absolute left-0 right-0 top-[calc(100%+0.5rem)] z-30 max-h-56 overflow-y-auto rounded-2xl bg-white p-1 shadow-float ring-1 ring-black/[0.05]"
          >
            {matches.length ? (
