...
async function loadWorldCoordinates() {
  // A heavy SPARQL query that sometimes times out, and more often comes back
  // truncated mid-JSON. Coordinates are an improvement, not a requirement, so a
  // bad day at Wikidata must not stop the dataset being rebuilt — but a single
  // truncated response silently costing every non-US school its position is
  // worse, so it is worth asking again before giving up.
  for (let attempt = 1; attempt <= 4; attempt++) {
    try {
      return await fetchWorldCoordinates();
    } catch (error) {
      console.log(`Wikidata attempt ${attempt} failed (${error.message})`);
      if (attempt < 4) await new Promise((done) => setTimeout(done, attempt * 2000));
    }
  }
  console.log("Wikidata unreachable; skipping world coordinates");
  return new Map();
}
...
// The world list is the only source that carries email domains, which is what
// lets an account on cs.stanford.edu recognise its own school.
...
for (const entry of world) {
  ...
  // A school listed twice is one school with two sets of domains. Taking the
  // union beats keeping whichever happened to come first.
  const existing = colleges.get(key);
  if (existing) {
    for (const domain of domainsOf(entry)) existing.domains.add(domain);
    continue;
  }
  ...
  colleges.set(key, {
    ...
    enrollment: 0,
    aliases: new Set(),
    domains: new Set(domainsOf(entry)),
  });
}
...
// ${sorted.length} schools, one per line: name, country, region, aliases, town, lat, lon, size, domains.
