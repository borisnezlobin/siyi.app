...
describe("ranking people in the picker", () => {
  ...
  it("shows whoever was seen most recently when nothing is typed", () => {
    const results = rankPeopleForPicker(
      [
        person("1", "Amelia Chen", {
          lastInteractionAt: "2026-01-01T00:00:00Z",
        }),
        person("2", "Luis Ortega", {
    ...
    expect(namesOf(results)).toEqual([
      "Luis Ortega",
      "Amelia Chen",
      "Never Met",
    ]);
  ...
  it("keeps two people who have never been logged in a stable order", () => {
    const results = rankPeopleForPicker(
      [person("1", "Zoe Winters"), person("2", "Ada Bell")],
      "",
    );

    expect(namesOf(results)).toEqual(["Ada Bell", "Zoe Winters"]);
  });
