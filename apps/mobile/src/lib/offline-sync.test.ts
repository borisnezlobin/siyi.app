...
describe("before the migrations have been applied", () => {
  beforeEach(async () => {
    await AsyncStorage.clear();
    mintedIds = 0;
    database = new FakeDatabase();
    database.missingTables.add("person_notes");
    database.missingTables.add("person_contact_methods");
    (supabase.from as jest.Mock).mockImplementation((table: string) =>
      database.from(table),
    );
    await goOffline();
  ...
  it("still saves the person, and clears the queue rather than retrying forever", async () => {
    database.seed("people", [{ id: personId, user_id: userId }]);
    await seedSnapshot();
    ...
    await updatePerson(
      ...
      [
        ...
        { kind: "phone", value: "(555) 555-0124", label: null, isPrimary: false },
      ],
      [],
    );
  ...
  it("does not get stuck on a queued note edit", async () => {
    ...
    await deletePersonNote(userId, note({ id: "note-2" }));
